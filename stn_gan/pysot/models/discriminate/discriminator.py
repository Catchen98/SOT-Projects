...
class Discriminator(nn.Module):
    def __init__(self, in_channel, middle_channel):
        ...
        self.main = nn.Sequential(
            ...
            nn.LeakyReLU(0.2, inplace=True),
            # state size. 5*5
            nn.Conv2d(middle_channel * 8, middle_channel * 4, 3, 2, bias=False),
            nn.BatchNorm2d(middle_channel * 4),
            nn.LeakyReLU(0.2, inplace=True),
            # state size. (ndf*8) x 3 x 3
            nn.Conv2d(middle_channel * 4, middle_channel * 2, 3, 1, bias=False),
            nn.BatchNorm2d(middle_channel * 2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(middle_channel * 2, 1, 5, 1, bias=False),
            nn.Sigmoid()
        )
