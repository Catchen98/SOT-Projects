...
class SubDataset(object):
    ...
    def get_positive_pair(self, index):
        ...
        if template_frame >= len(frames)-10 and template_frame>10:
            left = max(template_frame - self.frame_range, 0)
            right = template_frame-9
        elif template_frame <= 10 and len(frames)-template_frame >10:
            left = template_frame+10
            right = min(template_frame + self.frame_range, len(frames)-1)+1
        else:
            left = max(template_frame - self.frame_range, 0)
            right = min(template_frame + self.frame_range, len(frames)-1) + 1
