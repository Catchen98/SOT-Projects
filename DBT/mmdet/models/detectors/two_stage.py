...
class TwoStageDetector(BaseDetector, RPNTestMixin, BBoxTestMixin,
    ...
    def forward_train(self,
        ...
        if self.with_agg:
            ...
            #     support_id=random.randint(0,4)
            #     while img_meta[reference_id]['filename'].split('/')[-2] != \
            #         img_meta[support_id]['filename'].split('/')[-2]:
            #         support_id=random.randint(0,4)
    ...
    def simple_test(self, img, img_meta, proposals=None, rescale=False):
        ...
        if not self.with_mask:  
            ...
            # return bbox_results,det_labels
        else:
            segm_results = self.simple_test_mask(
                x, img_meta, det_bboxes, det_labels, rescale=rescale)
            return bbox_results, segm_results
        
    def predict_boxes(self, images, boxes,img_meta):
        ...
        proposals = [boxes]####将上一帧的检测框作为proposal
        ...
        # print('predict')
        # embed()
        pred_boxes, pred_labels = self.simple_test_bboxes(
            features, img_meta, proposals, rcnn_test_cfg=self.test_cfg.trcnn, rescale=False)
        
        # pred_boxes = bbox2result(pred_boxes, pred_labels,
        #                            self.bbox_head.num_classes)
