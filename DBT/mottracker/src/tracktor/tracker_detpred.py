...
class Tracker(BaseDetector, RPNTestMixin, BBoxTestMixin,
                       MaskTestMixin):
		...
		if self.do_reid:
			for t in self.inactive_tracks:
				if t.last_v.nelement() > 0:
					self.motion_step(t)
	def checkbackground(self,bboxes,labels):
		...
		for i, label in enumerate(labels):
			if label!=0:
				results_box.append(bboxes[i].view(1,-1).tolist())
				results_label.append(label.view(1,-1))
		return results_box,results_label
	def regress_tracks(self, img,img_metas):
		"""Regress the position of the tracks and also checks their scores."""
		pos = self.get_pos()
		# print('pos:',len(pos))
		# regress 基于上一帧的检测框作为proposal
		boxes, labels = self.obj_detect.module.predict_boxes(img, pos,img_metas.data[0])
		
		s = []
		for i in range(len(self.tracks) - 1, -1, -1):
			t = self.tracks[i]
			t.score = boxes[i,-1]
			if boxes[i,-1] <= self.regression_person_thresh:
				self.tracks_to_inactive([t])
			else:
				s.append(boxes[i,-1])
				# t.prev_pos = t.pos
				t.pos = boxes[i,0:4].view(1, -1)
	...
	def step(self, blob):
		...
		# print('step:',type(blob['img'][0]))
		boxes,labels = self.obj_detect(return_loss=False, rescale=not False,**blob)
		# boxes,labels = self.checkbackground(boxes,labels)
		# self.obj_detect.module.show_result(blob,boxes)
		# embed()
		# exit()
		if boxes.nelement() > 0:
			# boxes = clip_boxes_to_image(boxes[...,0:4], blob['img'][0].shape[-2:])

			# Filter out tracks that have too low person score
			inds = torch.gt(boxes[...,-1], self.detection_person_thresh).nonzero().view(-1)
		else:
			inds = torch.zeros(0).cuda()

		if inds.nelement() > 0:
			det_pos = boxes[inds,0:4]
			det_labels = labels[inds]
			det_scores = boxes[inds,-1]
		else:
			det_pos = torch.zeros(0).cuda()
			det_scores = torch.zeros(0).cuda()
			det_labels = torch.zeros(0).cuda()
		# print('det',len(det_pos))
		# embed()
		# self.reset()
		# self.add(det_pos,det_scores,None)
		# for t in self.tracks:
		...
		if len(self.tracks):
			...
			# regress
			person_scores = self.regress_tracks(blob['img'][0],blob['img_meta'][0])
			# embed()
			# self.add(person_scores[...,0:4],person_scores[...,-1],None)
			# for t in self.tracks:
			# 	if t.id not in self.results.keys():
			# 		self.results[t.id] = {}
			# 	self.results[t.id][self.im_index] = np.concatenate([t.pos[0].cpu().numpy(), np.array([t.score])])
		...
		if det_pos.nelement() > 0:
			...
			# print('new_det',len(new_det_pos))
			# add new
			if new_det_pos.nelement() > 0:
				self.add(new_det_pos, new_det_scores, None)
