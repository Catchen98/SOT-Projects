...
class ModelBuilder(nn.Module):
    ...
    def forward(self, data,idx):
        ...
        search = torch.zeros(template.size(0), template.size(1), 255, 255).cuda()
        search[:, :, :127, :127] = template
        save_search = search[0, :, :, :].detach().cpu().numpy()
        ...
        if cfg.DISENTANGLE.DISENTANGLE:
            results = self.split(zf, xf)
        if cfg.RECONSTRUCTION.RECONSTRUCTION:
            k_mask_img = self.reconstruct_mask(results['k_mask'])
            s_mask_img = self.reconstruct_mask(results['s_mask'])
            k_image = self.reconstruct_image(results['k_img'])
        ...
        if cfg.DISCRIMINATOR.DISCRIMINATOR:
            ...
            # s_realscore = self.discriminate(search/255 * s_mask_img)
            # ks_reconsscore = self.discriminate(ks_image * s_mask_img)
        # embed()
        # -------------mask_loss
        ...
        # s_mask_loss = s_mask_img.view(s_mask_img.size(0), -1)
        # smask_gt = torch.zeros(template.size(0), 1, 255, 255).cuda()
        # smask_gt[:, :, :127, :127] = (template_mask//255)[:, 0, :, :].unsqueeze(1)
        # smask_gt = smask_gt.view(template.size(0), -1)
        # smask_gt = smask_gt.type(torch.float)
        # smask_loss = nn.BCELoss()(s_mask_loss, smask_gt)
        # k_mask_img = F.softmax(k_mask_img, dim=1)
        # s_mask_img = F.softmax(s_mask_img, dim=1)
        # mask_gt = (template_mask//255)[:, 0, :, :]
        # mask_gt = mask_gt.type(torch.long)
        # mask_loss = F.nll_loss(k_mask_img, mask_gt)
        # -------------reconstruct_loss
        ...
        # sr_d_loss = discriminate_loss(s_realscore, real_label)
        # ksf_d_loss = discriminate_loss(ks_reconsscore, fake_label)
        kf_g_loss = discriminate_loss(k_reconsscore, real_label)
        # ksf_g_loss = discriminate_loss(ks_reconsscore, real_label)
        ...
        D_loss = kr_d_loss + kf_d_loss #+ sr_d_loss + ksf_d_loss
        G_loss = kf_g_loss + restruct_loss + mask_loss #+ smask_loss#+ ksf_g_loss
        ...
        save_k_mask = np.int64(save_k_mask > 0.5)
        # save_k_mask = k_mask_img[0, :, :, :].detach().cpu().numpy()
        # save_k_mask = np.argmax(save_k_mask, axis=0)
        save_k_mask = (save_k_mask * 255)

        save_s_mask = s_mask_img[0, 0, :, :].detach().cpu().numpy()
        save_s_mask = np.int64(save_s_mask > 0.5)
        # save_s_mask = s_mask_img[0, :, :, :].detach().cpu().numpy()
        # save_s_mask = np.argmax(save_s_mask, axis=0)
        save_s_mask = (save_s_mask * 255)
        ...
        # save_search = (data['search'][0, :, :, :]).detach().cpu().numpy()
        ...
        save_search_mask = save_search + save_s_mask
        save_search_mask = save_search_mask/np.max(save_search_mask)
        save_search_mask = (save_search_mask*255)
        ...
        save_search_mask = np.uint8(save_search_mask.transpose(1, 2, 0))
        ...
        if idx % 200 == 0:
            ...
            cv2.imwrite(saveimage_path + '/s_mask_{}.jpg'.format(idx), save_s_mask)
            ...
            cv2.imwrite(saveimage_path + 'search_mask_{}.jpg'.format(idx), save_search_mask)
